...
# Πώς να χρησιμοποιήσετε το block `comfor-latest` του Ascoos Cms.

Αυτό το σεμινάριο χωρίζεται σε τέσσερα βήματα.
1) Τι είναι το πρόγραμμα «`Comfor`».
2) Εγκατάσταση του block «`comfor-latest`».
...
2) πηγαίνουμε στο τμήμα `Views` και από το drop-down με την ονομασία `Block` επιλέγουμε το `comfor-latest`. Αυτό θα μας εμφανίσει στην λίστα των blocks μόνο το block με τίτλο `Comfor Latest Messages`.
